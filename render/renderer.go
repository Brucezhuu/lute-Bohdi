...
type Options struct {
	...
	AutoSpace bool
	// RenderListStyle 设置在渲染 OL、UL 时是否添加 data-style 属性 https://github.com/Brucezhuu/lute-Bohdi/issues/48
	...
	// SuperBlock 设置是否支持超级块。 https://github.com/Brucezhuu/lute-Bohdi/issues/111
	SuperBlock bool
	// ImageLazyLoading 设置图片懒加载时使用的图片路径，配置该字段后将启用图片懒加载。
	// 图片 src 的值会复制给新属性 data-src，然后使用该参数值作为 src 的值 https://github.com/Brucezhuu/lute-Bohdi/issues/55
	ImageLazyLoading string
	// ChineseParagraphBeginningSpace 设置是否使用传统中文排版“段落开头空两格”。
	ChineseParagraphBeginningSpace bool
	// Sanitize 设置是否启用 XSS 安全过滤 https://github.com/Brucezhuu/lute-Bohdi/issues/51
